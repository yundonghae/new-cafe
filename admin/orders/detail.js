/* ===== 관리자 주문 상세 ===== */
const { $, formatPrice, formatDateTime, escapeHtml, getParam, showToast, ORDER_STATUS, getOrderById, updateOrderStatus } =
  window.CafeUtils;
...
function render() {
  ...
  const options = Object.entries(ORDER_STATUS)
    .map(([sid, s]) => `<option value="${sid}" ${sid === order.status ? "selected" : ""}>${escapeHtml(s.label)}</option>`)
    .join("");

  root.innerHTML = `
    <section class="order-summary card">
      <div class="order-summary__row"><span>주문번호</span><span>${escapeHtml(order.id)}</span></div>
      <div class="order-summary__row"><span>주문일시</span><span>${formatDateTime(order.createdAt)}</span></div>
      <div class="order-summary__row">
        <span>상태</span>
        <select class="select" id="statusSelect">${options}</select>
      </div>
    </section>
